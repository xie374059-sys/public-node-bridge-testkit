#!/usr/bin/env python3
"""Send one safe sandbox file-delivery probe to Node-C and wait for result."""
...
DEFAULT_MARKER = "NODEC-FILE-001"
DEFAULT_FILENAME = "nodec_file_preflight_001.txt"
DEFAULT_CONTENT = "YUANJIE_NODEC_FILE_PREFLIGHT_001\nsafe=sandbox_only\nexecute=false\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Send one safe Node-C file-delivery probe.")
    ...
    while time.monotonic() < deadline:
        ...
        if task.get("status") == "completed":
            ok = (
                ...
                and result.get("execution") == "local_adapter_file_deliver_sandbox_write"
            )
            print(json.dumps({
                "ok": ok,
                "relay": relay,
                ...
                "claim": "node_c_file_channel_preflight_passed" if ok else "node_c_file_channel_result_mismatch",
                "cannot_claim": [
                    "real_codex_ipc",
                    "formal_ack",
                    "external_send",
    ...
    print(json.dumps({
        ...
        "status": "timeout",
        "latest": latest,
        "claim": "node_c_file_channel_preflight_timeout",
        "cannot_claim": [
            "real_codex_ipc",
            "formal_ack",
            "external_send",
